...
void dump_Sw(COMP Sw[]) {
	...
	fprintf(fsw,"%f\t",
		10.0f*log10f(Sw[i].real*Sw[i].real + Sw[i].imag*Sw[i].imag));
    fprintf(fsw,"\n");
}
...
void dump_Sw_(COMP Sw_[]) {
    ...
    for(i=0; i<FFT_ENC/2; i++)
	fprintf(fsw_,"%f\t",
		10.0f*log10f(Sw_[i].real*Sw_[i].real + Sw_[i].imag*Sw_[i].imag));
    fprintf(fsw_,"\n");
}
...
void dump_Ew(COMP Ew[]) {
    ...
    for(i=0; i<FFT_ENC/2; i++)
	fprintf(few,"%f\t",
		10.0f*log10f(Ew[i].real*Ew[i].real + Ew[i].imag*Ew[i].imag));
    fprintf(few,"\n");
}
